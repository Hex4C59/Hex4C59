...
整个系统的"大脑"。负责理解目标、生成推理、决定下一步动作。它不直接执行任何操作，而是输出结构化的决策——调用哪个工具、传什么参数、还是直接返回结果。

### 2. Memory（记忆模块）

负责维护 Agent 的上下文状态。通常分为两类：短期记忆是当前任务的对话历史和中间结果，存在上下文窗口里；长期记忆则持久化存储，通常借助向量数据库实现语义检索。

### 3. Tools（工具集）

Agent 与外部世界交互的唯一通道。每个工具有明确的接口定义（名称、参数、返回值），LLM Core 通过工具调用的方式驱动实际操作，比如搜索、读文件、执行代码、访问 API 等。

### 4. Planning（规划模块）

负责把复杂目标分解成可执行的子任务序列。它可以是独立的规划步骤（比如 ReAct 模式里的 Thought），也可以内嵌在每轮 LLM 推理中动态生成。
...
![Agent 典型组件协作与数据流示意](agent_components.svg)
